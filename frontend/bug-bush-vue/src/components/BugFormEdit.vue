<template >
  <h1 class="text-dark text-3xl font-bold" > Edit Bug </h1>
  <div class="flex flex-row justify-center p-4 ">
    ...
    <form @submit.prevent="submitBug" class="z-1 bg-light p-4 rounded-md shadow-2xl mb-6 flex flex-col gap-4 w-full min-w-sm max-w-4xl border-3 border-dashed border-accent">
      ...
      <button type="submit" id="tag-btn" class="bg-accent text-dark py-2 px-4 rounded hover:bg-primary hover:cursor-pointer">Update Bug</button>
    </form>
  </div>
...
let currentBugID = 0
// getting the bug details for the bug id for it to be populated
const fetchBugDetails = async (bugId) => {
  ...
  if (res.ok) {
    ...
    emoji.value = bug.emoji
    solution.value = bug.solution
    isSolutionVisible.value = bug.status === 'Closed'
    tags.value = bug.tags || []
  } else {
    console.error('Failed to fetch bug details:', res.statusText)
  }
}
...
const submitBug = async () => {
  const res = await fetch(`/api/bugs/${currentBugID}`, {
    method: 'PUT',
  ...
  if (res.ok) {
    ...
    console.error('Failed to update bug:', res.statusText)
  }

  // need to also update the tags here and in normal add tags to the tags table

  // Clear form -> or could just immediately redirect back to the bugs page
  title.value = ''
  description.value = ''
